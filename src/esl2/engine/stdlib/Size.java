...
public final class Size extends StandardUnaryFunction
{
    ...
    public ValueType fun(ValueType arg) throws TypedOperationException
    {
        ...
        {
            return new DoubleValue(((ArrayValue)arg).value.size());
        }
        else if (arg instanceof DictionaryValue)
        {
            return new DoubleValue(((DictionaryValue)arg).value.size());
        }
        else
        {
            throw new TypedOperationException("Error getting size of non-Collection.");
        }
    }
