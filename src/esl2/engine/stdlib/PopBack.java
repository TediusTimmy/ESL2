...
public final class PopBack extends StandardUnaryFunction
{
    ...
    public ValueType fun(ValueType arg) throws TypedOperationException
    {
        if (arg instanceof ArrayValue)
        {
            ...
            {
                ArrayValue result = new ArrayValue(new ArrayList<ValueType>(src.value.subList(0, src.value.size() - 1)));
                return result;
            }
            else
            {
                throw new TypedOperationException("Error popping back of empty Array.");
            }
        }
        else
        {
